...
function validarFormulario(){
    if(validarDni() & validarNombre() & validarApellidos() & validarDireccion() 
        ...
        document.getElementById("formularioProfesor").submit();
    }else{
...
function validarContrasenya(){
    ...
    if(!campoVacio(contrasenya, errorContrasenya)){
        contrasenya.classList.remove("error");
        errorContrasenya.innerHTML = "";
        return true;
    }else{
        return false;
    };
...
function validarExpediente(){
    let errorLicencia = document.getElementById("errorLicencia");
    
    if(!campoVacio(licencia, errorLicencia)){
        let expresion = /^\d{8}$/;
        let mensaje = "El expediente ha de tener 8 dígitos";
        
        return comprobarExpresion(licencia, errorLicencia, expresion, mensaje);
    }
}
